...
class NGramBuilder:
    def __init__(self, vocab_file: str, ngram_threshold: int = 4, nmin: int = 3, nmax: int = 5, seed: int = None):
        ...
        self.ngram_in_word_counts = {UNK_ID: 0, PAD_ID: 0} #added by Raj, want some frequency score of subwords 
        ...
        for (ngram, count) in most_common:
            if count >= ngram_threshold:
                ...
                self.ngram_in_word_counts[id_] = count #added by Raj, want a frequency feature of subwords
...
class BagOfNgrams(nn.Module):
    ...
    #added by Raj : don't avg the ngrams, return the list
    def forward_ngram_all_embs(self, ngram_ids, ngram_lengths):
        """
        :param ngram_ids:       shape is [batch_size x max_seq_length]
        :param ngram_lengths:   shape is [batch_size]
        """
        ...
        return bag_of_ngrams, ngrams_embedded, mask