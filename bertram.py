...
class BertramConfig:
    ...
    def __init__(self, transformer_cls: str, output_size: int, mode: str, ngram_vocab_size: int, transformer_dropout=0.1, variant_num_layers=1, variant_sub_num_layers=8, gate_combiner=''):
        assert transformer_cls in MODELS.keys()
        assert mode in MODES
        ...
        self.variant_num_layers=variant_num_layers
        self.variant_sub_num_layers=variant_sub_num_layers
        self.gate_combiner = gate_combiner
...
class ReliabilityMeasure(nn.Module):
    ...
    #added by Raj for cross attention 
    def forward_cross(self, inp1, mask1, inp2, mask2):
        inp1 = self.linear(inp1)

        inp2 = self.linear(inp2)
        
        similarities = self._get_context_sub_cross_similarities(inp1, inp2)
        similarities = self._mask_context_similarities2(similarities, mask1, mask2)
        reliabilities = self._get_context_reliabilities(similarities)
        return reliabilities
    ...
    @staticmethod
    def _mask_context_similarities2(similarities, mask, mask2):
        mask_key = mask2.unsqueeze(1)
        mask_query = mask.unsqueeze(2)
        return similarities * mask_key * mask_query
    ...
    #This is cross attention using the AM framework
    #maybe it works better for some reason?  Try it out!
    #make sure you handle masking correctly
    @staticmethod
    def _get_context_sub_cross_similarities(inp_mode1, inp_mode2):
        """
        :param contexts: (batch_size x max_context_length x emb_dim)
        """
        ...
        Q = inp_mode1
        K = torch.transpose(inp_mode2, dim0=-2, dim1=-1)
        d_k = torch.tensor(inp_mode1.shape[-1], dtype=torch.float)
        similarities = torch.matmul(Q, K) / torch.sqrt(d_k)
        
        #similarities[~mask2] = 0.0  # or any other value you want to use for masking
        #similarities = similarities * mask2.unsqueeze(1)
...
class Bertram(BertPreTrainedModel):
    ...
    def setup(self):
        ...
        if form_and_context and not isinstance(self.transformer.embeddings.word_embeddings, OverwriteableEmbedding): #see https://github.com/timoschick/bertram/issues/5
...
class BertramWrapper:
    ...
    def add_word_vectors_to_model(self, words_with_contexts: Dict[str, List[str]], tokenizer: PreTrainedTokenizer,
        ...
        if embedding_weight.shape[0] <= max_id:
            ...
            new_embd = torch.nn.Parameter(torch.cat([embedding_weight, filler]).detach(), requires_grad=False)
    ...
    #added by Raj, small adjustment to above
    def add_direct_word_vectors_to_model(self, embedding_dict, tokenizer: PreTrainedTokenizer,
                                  model: PreTrainedModel) -> None:
